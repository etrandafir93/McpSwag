...
class SpecParser(schemaConverter: SchemaConverter):
  ...
  def parse(source: SpecSource): List[OperationDef] =
    ...
    val result: SwaggerParseResult =
      if location.startsWith("classpath:") then
        ...
        if !res.exists() then
          throw new RuntimeException(s"Classpath resource not found: $resourcePath")
    ...
    val openApi = Option(result.getOpenAPI).getOrElse {
      ...
      throw new RuntimeException(s"Failed to parse spec '$specName' from $location: $msgs")
    }
  ...
  private def toOperationDef(
    ...
    val operationId = Option(op.getOperationId).filter(_.nonEmpty)
      .getOrElse(synthesizeOperationId(method, path))
    ...
    OperationDef(
      ...
      method = method,
      path = path,
      summary = Option(op.getSummary).filter(_.nonEmpty),
      description = Option(op.getDescription).filter(_.nonEmpty),
      parameters = params,
